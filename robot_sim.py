...
cube = p.loadURDF("cube_small.urdf", [0.45, 0.1, 0.02]) 
...
def get_system_prompt(cube_pos):
    return f"""You are a motion planner for an xArm6 robot on a floor.
Current Cube Position: {cube_pos}
Transit Height: z={TRANSIT_Z}

RULES:
1. Start every plan by rising to z={TRANSIT_Z}.
2. Travel to the cube's X,Y while staying at z={TRANSIT_Z}.
3. Only then descend to z=0.03 to grab.
    ...
    while True:
        ...
        try:
            response = client.messages.create(
                ...
                max_tokens=1024,
                system=get_system_prompt(list(cube_pos)),
...
print("\n--- ROBOT READY ---")
print("Commands: 'pick up the cube', 'go home', 'move right 20cm'")
...
while True:
    ...
    final_z = max(current_target[2], 0.01)
    final_target = [current_target[0], current_target[1], final_z]
    ...
    for i, joint_idx in enumerate(MOVABLE_JOINTS[:6]): # Limit to the 6 arm joints
        p.setJointMotorControl2(robot, joint_idx, p.POSITION_CONTROL, ik_angles[i])

    # Gripper Logic
    p.setJointMotorControl2(robot, GRIPPER_DRIVE, p.POSITION_CONTROL, current_gripper * 0.85)
